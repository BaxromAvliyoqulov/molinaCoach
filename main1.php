...
<html lang="en">
	<head>
		...
		<meta
			name="keywords"
			content="коуч, Ольга Молина, управление, тренерство, психология, значение, значения, ценностей, coach, Olga Molina, management, coaching, psychology, meaning, values"
		/>
	...
	<body>
		...
		<header style="background: transparent; padding-bottom: 10px !important ">
			...
			<div class="header_text container pt-5 mb-5">
				<div class="row pt-5">
					<div class="col pt-5">
						<div class="col-12 headerTextTest px-3">
							<p class="text-left mt-3 mb-2">
							Итак, вы отобрали те ценности, которые для вас, в принципе, ВАЖНЫ. Теперь выберите из них те, которые для вас ОЧЕНЬ ВАЖНЫ.
							<b>Карточки с очень важными ценностями должно быть не более 20!</b>
							</p>
						</div>
					</div>
				</div>
			...
			<div class="row card1 py-5">
				<?
					...
					if(isset($_COOKIE["username"])){ 
					    $cookie = explode(",",$_COOKIE["username"]); } else{ $cookie = []; } if (count($cookie)>0){ foreach ($data as
				$key1 => $value1) { if (in_array($value1['id'], $cookie)){ echo '
				<div
					id="'.$value1['id'].'"
